...
template <class T = std::size_t> class glist {
private:
...
public:
  ...
  template <typename Type = float> auto to_matrix() -> ten::tensor<Type> {
  ...
  template <class F> void dfs(const T &u, F f) {
    std::stack<T> stack;
    stack.push(u);
  ...
  template <class F> void bfs(const T &u, F f) {
    std::queue<T> queue;
    queue.push(u);
    std::unordered_set<T> visited;
    visited.insert(u);
