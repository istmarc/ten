#ifndef TEN_DISTRIBUTIONS_HXX
#define TEN_DISTRIBUTIONS_HXX
...
public:
  ...
  ten::tensor<T> sample(const std::vector<size_t> &shape,
                        bool requires_grad = false,
                        storage_order order = ::ten::storage_order::col_major) {
    ten::tensor<T> s(shape, storage_format::dense, requires_grad, order);
...
public:
  explicit normal(T mean = 0., T std = 1.) : _mean(mean), _std(std) {
...
public:
  explicit gamma(T alpha, T beta) : _alpha(alpha), _beta(beta) {
